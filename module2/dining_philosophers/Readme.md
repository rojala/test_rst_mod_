...
## Enhanced Features
This implementation includes advanced features beyond the basic dining philosophers problem:
- ✅ **Configurable philosophers and forks** via command-line options
- ✅ **Dynamic runtime additions** - add philosophers or forks during execution
- ✅ **Hunger level tracking** - monitor time since each philosopher's last meal
- ✅ **Priority system** - hungrier philosophers get priority access to forks
- ✅ **Real-time statistics** - display hunger metrics during simulation
- ✅ **Fairness metrics** - analyze eating patterns and wait times
...
Command line options added to modify amount of forks and philosophers.
``` bash
Usage: dining-philosophers [OPTIONS]

Options:
  -p, --philosophers <PHILOSOPHERS>          Number of philosophers [default: 15]
  -f, --forks <FORKS>                        Number of forks [default: 4]
```

### Can you implement a feature to dynamically add philosophers or forks during runtime?
Use the `--add-philosophers` and `--add-forks` options to schedule additions during runtime.
``` bash
Usage: dining-philosophers [OPTIONS]
...
Options:
  -p, --philosophers <PHILOSOPHERS>          Number of philosophers [default: 15]
  -f, --forks <FORKS>                        Number of forks [default: 4]
      --add-philosophers <TIME:COUNT,...>    Add philosophers after N seconds
      --add-forks <TIME:COUNT,...>           Add forks after N seconds
      --runtime <RUNTIME>                    Runtime duration in seconds
```

**Examples:**
[see](demo.sh)

**Format:** `TIME:COUNT` where TIME is seconds after start and COUNT is how many to add.

Multiple additions can be scheduled using comma separation: `"3:2,5:1,8:3"`

The program automatically calculates runtime to allow all scheduled additions to complete, or you can specify a custom runtime with `--runtime SECONDS`.

**How it works:**
- Philosophers and forks are stored in shared state using `Arc<RwLock<Vec<...>>>`
- Separate threads monitor the schedule and add resources at specified times
- New philosophers are assigned to existing forks using modulo arithmetic
- Philosophers continuously eat until the `running` flag is set to false
- The simulation tracks and reports the final count of philosophers and forks

### How would you measure the "hunger level" of each philosopher and implement a priority system?
Use the `--priority` flag to enable hunger-based priority system.
...
Options:
  ...
  -f, --forks <FORKS>                        Number of forks [default: 4]
      ...
      --add-forks <TIME:COUNT,...>           Add forks after N seconds
      --runtime <RUNTIME>                    Runtime duration in seconds
      --priority                             Enable priority system based on hunger level
      --stats <SECONDS>                      Display hunger statistics every N seconds
```

**Features:**
- **Hunger Tracking:** Measures time since each philosopher's last meal
- **Priority Queue:** Philosophers with higher hunger get priority access to forks
- **Statistics:** Use `--stats N` to display hunger statistics every N seconds
- **Metrics Tracked:**
  - Current hunger level (time since last meal)
  - Times eaten
  - Average wait time
  - Total wait time
...
# Enable priority with statistics every 3 seconds
cargo run -- -p 5 -f 2 --priority --stats 3 --runtime 10

# Combined with dynamic additions
cargo run -- -p 3 -f 2 --priority --stats 2 --add-philosophers "3:2" --runtime 8
```

**Examples:**
[see](demo_priority.sh)

**How it works:**
- Each philosopher tracks their `last_meal_time` and continuously updates their `current_hunger`
- When acquiring forks, philosophers add themselves to a priority queue sorted by wait time
- Only the philosopher with the longest wait time (highest priority) can acquire the fork
- Statistics thread periodically displays the top 10 hungriest philosophers
- Final summary shows total eating activity and fairness metrics

**More details**
[see](HUNGER_PRIORITY_SYSTEM.md)
